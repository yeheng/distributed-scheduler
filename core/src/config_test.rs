...
fn test_config_environment_override() {
    ...
    // 测试环境变量是否正确设置
    assert_eq!(env::var("SCHEDULER_DATABASE_MAX_CONNECTIONS").unwrap(), "50");
    ...
    println!("Dispatcher schedule_interval_seconds: {}", config.dispatcher.schedule_interval_seconds);
...
fn test_log_levels() {
    ...
    for level in &valid_levels {
        ...
        assert!(config.validate().is_ok(), "日志级别 {} 应该有效", level.to_uppercase());
    }
}