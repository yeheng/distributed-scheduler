...
use scheduler_foundation::{MessageQueue, SchedulerResult};
...
impl IntegrationTestSetup {
    ...
    pub async fn update_task_run_result(
        &self,
        task_run_id: i64,
        result: Option<&str>,
        error: Option<&str>,
...
async fn test_complete_task_lifecycle_integration() {
    ...
    let task = setup
        .create_test_task("integration_test_task", "shell")
        .await
        .unwrap();
    ...
    assert!(
        !messages.is_empty(),
        "Should have received task execution message"
    );
    ...
    if let Some(trace_headers) = consumed_message.get_trace_headers() {
        info!(
            "Message contains trace headers: {:?}",
            trace_headers.keys().collect::<Vec<_>>()
        );
    }

    // Step 6: Update task run status to Running
    setup
        .update_task_run_status(task_run.id, TaskRunStatus::Running, Some(&worker.id))
        .await
        .unwrap();
    ...
    setup
        .update_task_run_result(task_run.id, Some(execution_result), None)
        .await
        .unwrap();

    setup
        .update_task_run_status(task_run.id, TaskRunStatus::Completed, Some(&worker.id))
        .await
        .unwrap();
    ...
    let final_task_run = setup.get_task_run(task_run.id).await.unwrap().unwrap();
...
async fn test_task_dependency_integration() {
    ...
    // Create parent task
    let parent_task = setup
        .create_test_task("parent_task", "shell")
        .await
        .unwrap();

    // Create dependent task
    let mut dependent_task = setup
        .create_test_task("dependent_task", "shell")
        .await
        .unwrap();
    ...
    setup
        .update_task_run_status(parent_run.id, TaskRunStatus::Completed, None)
        .await
        .unwrap();
...
async fn test_error_handling_and_retry_integration() {
    ...
    let mut task = setup
        .create_test_task("failing_task", "shell")
        .await
        .unwrap();
    task.max_retries = 3;
    ...
    setup
        .update_task_run_result(task_run.id, None, Some("Command failed with exit code 1"))
        .await
        .unwrap();

    setup
        .update_task_run_status(task_run.id, TaskRunStatus::Failed, Some(&worker.id))
        .await
        .unwrap();

    // Verify task run is marked as failed
    let failed_run = setup.get_task_run(task_run.id).await.unwrap().unwrap();
    ...
    setup
        .update_task_run_status(retry_run.id, TaskRunStatus::Running, Some(&worker.id))
        .await
        .unwrap();

    setup
        .update_task_run_result(retry_run.id, Some("Task succeeded on retry"), None)
        .await
        .unwrap();

    setup
        .update_task_run_status(retry_run.id, TaskRunStatus::Completed, Some(&worker.id))
        .await
        .unwrap();

    let successful_retry = setup.get_task_run(retry_run.id).await.unwrap().unwrap();
...
async fn test_distributed_tracing_integration() {
    ...
    let _worker = setup
        .create_test_worker("tracing-worker", vec!["shell".to_string()])
        .await
        .unwrap();

    let task = setup
        .create_test_task("tracing_test_task", "shell")
        .await
        .unwrap();
    let task_run = setup.create_task_run(task.id).await.unwrap();
    ...
    let message_with_trace =
        Message::task_execution(task_execution_msg).inject_current_trace_context();
...
async fn test_message_queue_resilience() {
    ...
    for published_id in published_ids {
        assert!(
            consumed_ids.contains(&published_id),
