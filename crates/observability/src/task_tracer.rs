...
mod tests {
    ...
    fn test_span_creation_with_long_strings() {
        ...
        let long_queue_name = "very-long-queue-name-with-detailed-purpose-and-routing-information-1234567890";
        
        let span1 = TaskTracer::schedule_task_span(1, long_task_name, long_task_type);
        let span2 = TaskTracer::execute_task_span(1, 1, long_task_name, long_task_type, long_worker_id);
    ...
    fn test_span_creation_with_special_characters() {
        ...
        let span2 = TaskTracer::execute_task_span(1, 1, special_task_name, "shell", special_worker_id);
    ...
    fn test_record_error_with_different_error_types() {
        ...
        let custom_error = Box::<dyn std::error::Error + Send + Sync>::from(std::io::Error::new(std::io::ErrorKind::Other, "Custom error"));
    ...
    fn test_record_task_result_with_extreme_values() {
        ...
        TaskTracer::record_task_result(false, 100, Some("x".repeat(1000).as_str())); // Very long error message
    }
    ...
    fn test_multiple_span_creations() {
        ...
        for i in 0..5 {
            let span = TaskTracer::execute_task_span(i, i, "task", "type", &format!("worker_{}", i));
            let _ = span;
        }
    }
    ...
    fn test_database_span_with_various_operations() {
        ...
        let tables = vec![
            "tasks", "task_runs", "workers", "messages", "config", "metadata"
        ];
    ...
    fn test_message_queue_span_with_various_operations() {
        let operations = vec![
            "publish", "consume", "ack", "nack", "reject", "requeue"
        ];
        let queues = vec![
            "task_queue", "result_queue", "heartbeat_queue", "control_queue"
        ];
