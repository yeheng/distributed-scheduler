...
mod tests {
    ...
    impl MessageQueue for MockMessageQueue {
        async fn publish_message(&self, queue: &str, message: &Message) -> crate::Result<()> {
        ...
        async fn consume_messages(&self, queue: &str) -> crate::Result<Vec<Message>> {
            let mut queues = self.queues.lock().unwrap();
            let messages = queues.remove(queue).unwrap_or_default();
            Ok(messages)
        }

        async fn ack_message(&self, message_id: &str) -> crate::Result<()> {
            self.acked_messages
                .lock()
                .unwrap()
        ...
        async fn nack_message(&self, message_id: &str, _requeue: bool) -> crate::Result<()> {
        ...
        async fn delete_queue(&self, queue: &str) -> crate::Result<()> {
            let mut queues = self.queues.lock().unwrap();
            queues.remove(queue);
            Ok(())
        }

        async fn get_queue_size(&self, queue: &str) -> crate::Result<u32> {
            let queues = self.queues.lock().unwrap();
            let size = queues.get(queue).map(|q| q.len()).unwrap_or(0) as u32;
            Ok(size)
        }

        async fn purge_queue(&self, queue: &str) -> crate::Result<()> {
            let mut queues = self.queues.lock().unwrap();
