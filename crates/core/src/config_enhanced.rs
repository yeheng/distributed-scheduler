...
pub enum ConfigValidationError {
    ...
    InvalidValue { field: String, value: String, error: String },
...
pub trait ConfigValidator: Send + Sync {
    ...
    fn validate(&self, config: &serde_json::Value) -> std::result::Result<(), ConfigValidationError>;
...
pub struct BasicConfigValidator {
    ...
    custom_validators: HashMap<String, Box<dyn Fn(&serde_json::Value) -> std::result::Result<(), ConfigValidationError> + Send + Sync>>,
}
...
impl BasicConfigValidator {
    ...
    where
        F: Fn(&serde_json::Value) -> std::result::Result<(), ConfigValidationError> + Send + Sync + 'static,
    {
...
pub struct EnhancedConfigManager {
    ...
    file_watcher: Option<Arc<RecommendedWatcher>>,
...
impl EnhancedConfigManager {
        ...
        match source {
            ConfigSource::File { path } => {
                ...
                let content = fs::read_to_string(path)
                    .map_err(|e| SchedulerError::Configuration(format!(
    ...
    fn merge_configs(&self, base: serde_json::Value, override_config: serde_json::Value) -> serde_json::Value {
    ...
    /// Validate configuration
    async fn validate_config(&self, config: &serde_json::Value) -> std::result::Result<(), ConfigValidationError> {
    ...
    async fn setup_file_watcher(&self) -> Result<()> {
        ...
        ).map_err(|e| SchedulerError::Configuration(format!("Failed to create file watcher: {}", e)))?;
            ...
            if let ConfigSource::File { path } = source {
                if let Some(parent) = path.parent() {
                    watcher.watch(parent, RecursiveMode::NonRecursive)
                        .map_err(|e| SchedulerError::Configuration(format!("Failed to watch directory: {}", e)))?;
                }
            }
        }
    ...
    async fn reload_config(
        config: &Arc<RwLock<serde_json::Value>>,
        metadata: &Arc<RwLock<ConfigMetadata>>,
        validators: &[Box<dyn ConfigValidator>],
        callbacks: &[Box<dyn Fn(&serde_json::Value) + Send + Sync>],
    ) -> Result<()> {
                ...
                if let serde_json::Value::String(url) = value {
                    if url.starts_with("postgresql://") || url.starts_with("mysql://") || url.starts_with("sqlite://") {
                        ...
                        Err(ConfigValidationError::InvalidValue {
                            field: "url".to_string(),
                            value: url.clone(),
                            error: "Database URL must start with postgresql://, mysql://, or sqlite://".to_string(),
                        })
                    }
                } else {
                ...
                if let serde_json::Value::String(level) = value {
                    match level.to_lowercase().as_str() {
                        ...
                        _ => Err(ConfigValidationError::InvalidValue {
                            ...
                            value: level.clone(),
                            error: "Log level must be one of: trace, debug, info, warn, error".to_string(),
                        }),
                    }
                } else {
                    Err(ConfigValidationError::TypeMismatch {
...
mod tests {
    ...
    async fn test_environment_config() {
        ...
        let manager = EnhancedConfigManager::new()
            .add_source(ConfigSource::Environment { prefix: "TEST_".to_string() });
