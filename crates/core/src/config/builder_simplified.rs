...
use crate::config::{
    core::{ConfigurationService, TypedConfig, ConfigValue, ConfigSource},
    ...
    services::{ConfigCache as ServiceCache},
    builder::InMemoryConfigService,
};
...
impl ConfigManager {
    ...
    pub fn with_cache_ttl(mut self, ttl: std::time::Duration) -> Self {
        self.cache = ServiceCache::new(ttl);
        self
    }
    ...
    {
        TypedConfig::new(self.service.clone(), key.to_string())
    }
    ...
    pub async fn get_source(&self, key: &str) -> Result<Option<ConfigSource>, SchedulerError> {
