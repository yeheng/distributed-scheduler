...
impl MessageQueueConfig {
    ...
    /// 验证RabbitMQ配置
    fn validate_rabbitmq_config(&self) -> Result<()> {
        if self.url.is_empty() {
    ...
    /// 验证Redis Stream配置
    fn validate_redis_stream_config(&self) -> Result<()> {
        // 对于Redis Stream，URL可以为空（使用redis配置）或者是redis://格式
        if !self.url.is_empty()
    ...
    pub fn get_redis_url(&self) -> Option<String> {
        if self.is_redis_stream() {
            if !self.url.is_empty() {
                ...
                Some(format!(
                    "redis://{}{}:{}/{}",
                    auth, redis_config.host, redis_config.port, redis_config.database
                ))
            } else {
                None
            }
