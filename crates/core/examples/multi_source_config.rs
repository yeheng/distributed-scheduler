...
"#;
    ...
    let manager = ConfigBuilder::new()
        ...
        // 4. 命令行参数（最高优先级）
        .add_source(ConfigSource::CommandLine { args: cli_args })
        // 设置重载策略
    ...
    println!(
        "   服务器配置子集: {}",
        serde_json::to_string_pretty(&server_subset)?
    );
    println!(
        "   功能配置子集: {}",
        serde_json::to_string_pretty(&features_subset)?
    );
    ...
    // 更新配置值
    manager
        .set("dynamic.setting", serde_json::json!("dynamic_value"))
        .await?;
    manager
        .set("server.host", serde_json::json!("updated-host"))
        .await?;
...
"#;
    ...
    println!("   重载后的配置:");
    println!(
        "     主机: {} (应该还是 'cli-host'，因为命令行优先级更高)",
        reloaded_host
    );
    println!(
        "     端口: {} (应该还是 '5000'，因为环境变量优先级更高)",
        reloaded_port
    );
    println!(
        "     工作线程: {} (应该更新为 '16'，因为文件中没有冲突)",
        reloaded_workers
    );
