...
pub fn validate_task_name(name: &str) -> Result<(), ValidationError> {
    ...
    if !name.chars().all(|c| {
        c.is_alphanumeric() || c == '_' || c == '-' || c.is_whitespace() || c.is_ascii_punctuation() || ('\u{4e00}'..='\u{9fff}').contains(&c)
    }) {
...
pub fn validate_task_type(task_type: &str) -> Result<(), ValidationError> {
    ...
    if !task_type.chars().all(|c| {
        c.is_alphanumeric() || c == '_' || c == '.' || c == '-'
    }) {
        return Err(ValidationError::new("任务类型只能包含字母、数字、下划线、点和连字符"));
    }
...
pub fn validate_cron_expression(cron_expr: &str) -> Result<(), ValidationError> {
    ...
    match cron::Schedule::from_str(cron_expr) {
        Ok(_) => Ok(()),
        Err(e) => Err(ValidationError::new(&*Box::leak(format!("无效的CRON表达式: {}", e).into_boxed_str()))),
    }
}
    ...
    if let Some(obj) = params.as_object() {
        ...
        for key in obj.keys() {
            ...
            if !key.chars().all(|c| {
                ...
                return Err(ValidationError::new("参数键名只能包含字母、数字、下划线、点和连字符"));
            }
        }
...
pub fn validate_task_status(status: &str) -> Result<(), ValidationError> {
    ...
    match status.to_uppercase().as_str() {
        "ACTIVE" | "INACTIVE" => Ok(()),
        _ => Err(ValidationError::new("无效的任务状态，只能是 ACTIVE 或 INACTIVE")),
    }
}
...
pub fn validate_task_run_status(status: &str) -> Result<(), ValidationError> {
    ...
    match status.to_uppercase().as_str() {
        ...
        _ => Err(ValidationError::new("无效的任务运行状态，只能是 PENDING, DISPATCHED, RUNNING, COMPLETED, FAILED 或 TIMEOUT")),
    }
}
...
pub async fn validate_create_task_request(
    ...
    if contains_sql_injection_risk(&request.name) {
        return Err(crate::error::ApiError::BadRequest("任务名称包含潜在的安全风险".to_string()));
    }
    
    // 检查任务类型是否为系统保留类型
    if is_reserved_task_type(&request.task_type) {
        return Err(crate::error::ApiError::BadRequest("不能使用系统保留的任务类型".to_string()));
    }
    
    // 验证CRON表达式是否合理（不能过于频繁）
    if is_cron_too_frequent(&request.schedule)? {
        return Err(crate::error::ApiError::BadRequest("任务调度频率过高，请调整CRON表达式".to_string()));
    }
    
    // 检查参数中的命令注入风险
    if contains_command_injection_risk(&request.parameters) {
        return Err(crate::error::ApiError::BadRequest("任务参数包含潜在的安全风险".to_string()));
    }
...
pub async fn validate_update_task_request(
    ...
    if let Some(ref deps) = request.dependencies {
        if check_circular_dependency(task_id, deps, state).await? {
            return Err(crate::error::ApiError::BadRequest("检测到循环依赖".to_string()));
        }
    }
    ...
    if let Some(dependencies) = deps {
        for &dep_id in dependencies {
            ...
            match state.task_repo.get_by_id(dep_id).await {
                Ok(Some(_)) => continue,
                Ok(None) => {
                    return Err(crate::error::ApiError::BadRequest(format!("依赖任务 {} 不存在", dep_id)));
                }
                Err(e) => {
                    tracing::error!("Failed to check dependency task {}: {:?}", dep_id, e);
                    return Err(crate::error::ApiError::Internal("验证依赖时发生错误".to_string()));
                }
            }
        }
    }
...
/// 检查敏感参数
fn check_sensitive_parameters(obj: &serde_json::Map<String, serde_json::Value>) -> Result<(), ValidationError> {
    let sensitive_keys = [
    ...
    for key in obj.keys() {
        ...
        if sensitive_keys.iter().any(|sensitive| lower_key.contains(sensitive)) {
...
fn contains_sql_injection_risk(text: &str) -> bool {
    let sql_keywords = [
        ...
        "UNION", "WHERE", "OR", "AND", "EXEC", "EXECUTE", "CAST", "CONVERT"
    ];
    
    let upper_text = text.to_uppercase();
    sql_keywords.iter().any(|&keyword| upper_text.contains(keyword))
}
...
fn is_reserved_task_type(task_type: &str) -> bool {
    let reserved_types = [
        "system", "admin", "root", "scheduler", "internal",
        "__system__", "__internal__", "__admin__"
    ];
    
    let lower_type = task_type.to_lowercase();
    reserved_types.iter().any(|&reserved| lower_type.starts_with(reserved))
}
...
fn is_cron_too_frequent(cron_expr: &str) -> Result<bool, ValidationError> {
    match cron::Schedule::from_str(cron_expr) {
        Ok(schedule) => {
            ...
            while let Some(current_time) = next_executions.next() {
        ...
        Err(e) => Err(ValidationError::new(&*Box::leak(format!("CRON表达式解析失败: {}", e).into_boxed_str()))),
    }
}
...
async fn check_circular_dependency(
    ...
    while let Some(current_id) = queue.pop_front() {
        ...
        match state.task_repo.get_by_id(current_id).await {
            ...
            Err(e) => {
                tracing::error!("Failed to check dependency for task {}: {:?}", current_id, e);
                return Err(crate::error::ApiError::Internal("检查循环依赖时发生错误".to_string()));
            }
        }
