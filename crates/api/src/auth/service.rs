use crate::auth::{AuthConfig, AuthenticatedUser, AuthType, Claims};
...
impl AuthService {
    pub fn new(config: &AuthConfig) -> Self {
        Self {
            jwt_service: crate::auth::JwtService::new(&config.jwt_secret, config.jwt_expiration_hours),
            user_service: crate::auth::models::UserService::new(),
        }
    }

    pub async fn login(&self, username: &str, password: &str) -> Result<crate::auth::models::LoginResponse, String> {
        let user = self.user_service.authenticate_user(username, password).await?;
        
        let permissions = user.role.permissions();
        let access_token = self.jwt_service.generate_token(&user.id.to_string(), &permissions)
    ...
    pub async fn create_user(&mut self, request: crate::auth::models::CreateUserRequest) -> Result<crate::auth::models::UserResponse, String> {
        let user = self.user_service.create_user(request).await?;
        Ok(user.into())
    }

    pub async fn refresh_token(&self, refresh_token: &str) -> Result<crate::auth::models::LoginResponse, String> {
