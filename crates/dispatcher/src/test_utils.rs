...
pub mod mocks {
    ...
    impl TaskRepository for MockTaskRepository {
        ...
        async fn get_schedulable_tasks(&self, _now: DateTime<Utc>) -> Result<Vec<Task>> {
            Ok(vec![])
        }
    ...
    impl TaskRunRepository for MockTaskRunRepository {
        ...
        async fn get_by_worker_id(&self, _worker_id: &str) -> Result<Vec<TaskRun>> {
            Ok(vec![])
        }
        ...
        async fn cleanup_old_runs(&self, _keep_days: i32) -> Result<u64> {
            Ok(0)
        }

        async fn get_recent_runs(&self, _task_id: i64, _limit: i64) -> Result<Vec<TaskRun>> {
            Ok(vec![])
        }
        ...
        async fn get_execution_stats(
            ...
            task_id: i64,
            _days: i32,
        ) -> Result<scheduler_core::TaskExecutionStats> {
        ...
        async fn batch_update_status(&self, _ids: &[i64], _status: TaskRunStatus) -> Result<()> {
            Ok(())
        }
    }
    ...
    impl WorkerRepository for MockWorkerRepository {
        ...
        async fn get_workers_by_task_type(&self, _task_type: &str) -> Result<Vec<WorkerInfo>> {
            Ok(vec![])
        }
        ...
        async fn update_status(&self, _worker_id: &str, _status: WorkerStatus) -> Result<()> {
            Ok(())
        }
        ...
        async fn cleanup_offline_workers(&self, _offline_seconds: i64) -> Result<u64> {
            Ok(0)
        }
    ...
    impl RetryService for MockRetryService {
        async fn handle_failed_task(&self, _task_run_id: i64) -> Result<bool> {
            Ok(true)
        }

        async fn handle_timeout_task(&self, _task_run_id: i64) -> Result<bool> {
            Ok(true)
        }

        async fn handle_worker_failure(&self, _worker_id: &str) -> Result<Vec<TaskRun>> {
            Ok(vec![])
        }
