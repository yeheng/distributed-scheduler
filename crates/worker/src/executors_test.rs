#[cfg(test)]
mod tests {
    use crate::{ShellExecutor, HttpExecutor};
    ...
    async fn test_shell_executor_cleanup() {
        ...
        assert!(cleanup_result.is_ok(), "ShellExecutor cleanup should succeed");
    }
    ...
    async fn test_http_executor_cleanup() {
        ...
        // Call cleanup - should be a no-op but shouldn't fail
        let cleanup_result = executor.cleanup().await;
        assert!(cleanup_result.is_ok(), "HttpExecutor cleanup should succeed");
    }
    ...
    async fn test_mock_executor_cleanup() {
        ...
        // Call cleanup - should be a no-op but shouldn't fail
        let cleanup_result = executor.cleanup().await;
        assert!(cleanup_result.is_ok(), "MockTaskExecutor cleanup should succeed");
    }
