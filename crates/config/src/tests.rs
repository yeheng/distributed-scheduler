...
mod figment_tests {
    ...
    fn test_environment_variables() {
        ...
        use figment::{providers::{Serialized, Json, Format}, Figment};
        use serde_json::json;
    ...
    fn test_a_real_environment_variables_integration() {
        ...
        assert!(config.is_ok(), "default_with_env should work without environment variables");
        ...
        assert!(result.is_ok(), "from_env should work with development environment");
    }
    ...
    fn test_figment_env_parsing() {
        ...
        if let Ok(value) = figment2.find_value("database.max_connections") {
            println!("Found with __ separator database.max_connections: {:?}", value);
        } else {
