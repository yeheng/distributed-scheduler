...
impl ConfigValidator for DatabaseConfig {
    fn validate(&self) -> crate::ConfigResult<()> {
        ...
        if self.min_connections > self.max_connections {
            return Err(crate::ConfigError::Validation(
                "database.min_connections must be less than or equal to max_connections".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_database_config_serialization() {
        ...
        let deserialized: DatabaseConfig = serde_json::from_str(&serialized).expect("Failed to deserialize");
