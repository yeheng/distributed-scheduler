...
mod tests {
    ...
    fn test_resilience_config_default() {
        ...
        assert_eq!(config.database_circuit_breaker.recovery_timeout, std::time::Duration::from_secs(60));
        
        assert_eq!(config.message_queue_circuit_breaker.failure_threshold, 3);
        assert_eq!(config.message_queue_circuit_breaker.recovery_timeout, std::time::Duration::from_secs(30));
        
        assert_eq!(config.external_service_circuit_breaker.failure_threshold, 3);
        assert_eq!(config.external_service_circuit_breaker.recovery_timeout, std::time::Duration::from_secs(45));
    }
