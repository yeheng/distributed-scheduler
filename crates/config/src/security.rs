...
impl ConfigSecurity {
    ...
    pub fn encrypt_value(&self, value: &str) -> ConfigResult<String> {
        ...
        let mut sealing_key: aead::SealingKey<CounterNonceSequence> =
    ...
    pub fn decrypt_value(&self, encrypted_value: &str) -> ConfigResult<String> {
        ...
        let nonce_seq = CounterNonceSequence(0);
        let mut opening_key: aead::OpeningKey<CounterNonceSequence> =
            aead::BoundKey::new(unbound_key, nonce_seq);
    ...
    pub fn generate_secure_secret(&self, length: usize) -> ConfigResult<String> {
        if length < 32 {
            return Err(ConfigError::Security(
                "Secret length must be at least 32 characters".to_string(),
            ));
        }
            ...
            for (key, value) in config {
                if self.is_sensitive_key(key) {
                    if let Some(value_str) = value.as_str() {
                        ...
                        {
                            return Err(ConfigError::Security(format!(
                                "Potential insecure default value for sensitive key '{key}' in production"
                            )));
                        }
                    }
                }
        ...
        for setting in required_security_settings {
            if !config.contains_key(setting) {
                self.log_security_event(SecurityEvent {
                    ...
                    details: format!("Required security setting '{setting}' is missing"),
    ...
    pub fn import_secure_config(
        &self,
        secure_config_str: &str,
    ) -> ConfigResult<HashMap<String, serde_json::Value>> {
        let secure_config: HashMap<String, serde_json::Value> =
            serde_json::from_str(secure_config_str).map_err(|e| {
                ConfigError::Security(format!("Failed to parse secure config: {e}"))
            })?;
...
mod tests {
    ...
    fn test_encrypt_without_key() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Encryption key not configured"));
    }
    ...
    fn test_validate_security_requirements_production_insecure_defaults() {
        ...
        config.insert(
            "database_password".to_string(),
            serde_json::Value::String("test_password".to_string()),
        );
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("insecure default value"));
    }
    ...
    fn test_validate_security_requirements_missing_security_settings() {
        ...
        let has_security_violation = security
            .get_audit_log()
            .iter()
    ...
    fn test_validate_security_requirements_valid_config() {
        ...
        config.insert(
            "database_password".to_string(),
            serde_json::Value::String("secure_random_password_12345".to_string()),
        );
        config.insert(
            "api.auth.jwt_secret".to_string(),
            serde_json::Value::String("very_secure_jwt_secret_key".to_string()),
        );
        config.insert(
    ...
    fn test_load_environment_variables() {
        ...
        temp_env::with_vars(
            ...
            || {
                ...
                let has_sensitive_access = security.get_audit_log().iter().any(|event| {
                    matches!(
                        event.event_type,
                        SecurityEventType::EnvironmentVariableAccessed
                    )
                });
    ...
    fn test_load_environment_variables_sensitive_access_logging() {
        ...
        temp_env::with_var("TEST_CONFIG_PASSWORD", Some("secret_password"), || {
            ...
            // Check that sensitive variable access was logged
            let access_logs: Vec<_> = security
                .get_audit_log()
                .iter()
                .filter(|event| {
                    matches!(
    ...
    fn test_export_import_secure_config() {
        ...
        temp_env::with_var(
            ...
            || {
                ...
                assert_eq!(
                    imported.get("database_url").unwrap(),
        ...
        temp_env::with_var(
            ...
            || {
                ...
                assert!(result
                    .unwrap_err()
                    .to_string()
                    .contains("Failed to parse secure config"));
            },
        );
    }
